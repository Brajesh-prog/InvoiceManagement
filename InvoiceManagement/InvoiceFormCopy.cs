...
{
    public partial class InvoiceFormCopy : Form
    {
            ...
            if (e.ColumnIndex == 5 && e.RowIndex > -1)//HSN cell
            {
                ...
                foreach (DataGridViewRow cell in InvoicedataGrid.Rows)
                {
                    ...
                    if (ctr == InvoicedataGrid.Rows.Count)
                    {
                        ...
                        CGST.Text = Convert.ToDecimal((0.025 * SumAmount).ToString()).ToString(("0.##"));
                        SGST.Text = Convert.ToDecimal((0.025 * SumAmount).ToString()).ToString(("0.##"));
                        netTotal.Text = Math.Round(Convert.ToDecimal(SumAmount + (0.025 * SumAmount) + (0.025 * SumAmount))).ToString();
